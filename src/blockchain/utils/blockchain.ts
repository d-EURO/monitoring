 ...
 */
export function displayBlockProgress(eventName: string, processedBlocks: number, totalBlocks: number, complete: boolean = false): void {
	...
	const progressBar = complete ? '█'.repeat(20) : '█'.repeat(filledSections) + '='.repeat(20 - filledSections);
...
async function processSingleEventQuery<T extends BaseContract>(
	...
	for (let i = 0; i < blockRanges.length; i += concurrencyLimit) {
		...
		try {
			...
			const results = await processRangesWithReducedConcurrency(contract, eventFilter, currentBatch, reducedConcurrency);
...
export async function batchedEventQuery<T extends BaseContract>(
	...
	const getLatestBlock = async (): Promise<number> => {
		try {
			return await executeWithRetry(
				...
				2000,
				15000
			); // 3 retries, 2s delay, 15s timeout for block number query
		} catch (error) {
	...
	return queue.enqueue(() => processSingleEventQuery(contract, eventFilter, startBlock, latestBlock, chunkSize, concurrencyLimit));
}
